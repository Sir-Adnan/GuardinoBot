...
class Settings(BaseSettings):
    ...
    menu_title: str = "درگاه ریالی zibal"
...
from .clients import GatewayError, ZibalAPI

router = Router(name="payment/zibal")
...
class SettingsKeyboard(InlineKeyboardBuilder):
    class Callback(CallbackData, prefix="set_zibal"):
        field: Fields
...
class TextsKeyboard(InlineKeyboardBuilder):
    class Callback(CallbackData, prefix="txt_zibal"):
...
class ZibalEditForm(StatesGroup):
    api_key = State()
    menu_title = State()
...
class ZibalCustomAmountForm(StatesGroup):
    method = State()
    amount = State()


@router.message(
    StateFilter(ZibalEditForm),
    IsSuperUser(),
...
async def edit_settings(
    ...
    if _settings.enabled:
        ...
        text = "درگاه zibal غیرفعال شد!"
    else:
        _settings.enabled = True
        text = "درگاه zibal فعال شد!"
    await settings.Settings.update(payment_zibal=_settings)
...
async def edit_settings_texts(
    ...
    if callback_data.field == Fields.text_choose_amount:
        ed_text = _texts.payment_zibal.choose_amount
    elif callback_data.field == Fields.text_show_invoice:
        ed_text = _texts.payment_zibal.show_invoice
    else:
...
async def texts_reset(
    ...
    _texts = texts.get_texts().payment_zibal
    if callback_data.field == Fields.text_choose_amount:
    ...
    await texts.Texts.update(payment_zibal=_texts)
...
async def texts_edit(
    ...
    if callback_data.field == Fields.text_choose_amount:
        await state.set_state(ZibalEditForm.text_choose_amount)
    elif callback_data.field == Fields.text_show_invoice:
        await state.set_state(ZibalEditForm.text_show_invoice)
    else:
        return await query.answer(
...
@router.message(
    ZibalEditForm.text_choose_amount,
...
@router.message(
    ZibalEditForm.text_show_invoice,
    ~F.text.in_([CancelFormAdmin.cancel]),
    ~CommandStart(),
    ~Command("menu"),
    IsSuperUser(),
...
async def edit_settings(  # noqa: F811
    ...
    if callback_data.field == Fields.api_key:
        ...
        await state.set_state(ZibalEditForm.free_after)
        ...
        await state.set_state(ZibalEditForm.free_after_percent)
    else:
        return await query.answer(
...
@router.message(
    ZibalEditForm.free_after_percent,
    ~F.text.in_([CancelFormAdmin.cancel]),
    ~CommandStart(),
    ~Command("menu"),
    IsSuperUser(),
...
@router.message(
    ...
    ~CommandStart(),
    ~Command("menu"),
    StateFilter(ZibalCustomAmountForm),
)
@router.callback_query(
...
async def select(qmsg: CallbackQuery | Message, user: User, state: FSMContext = None):
    ...
    if not _settings.payment_zibal.enabled:
        text = f"🚫 در حال حاضر امکان پرداخت به وسیله {_settings.payment_zibal.menu_title} غیرفعال می‌باشد!"
        if isinstance(qmsg, CallbackQuery):
    ...
    text = texts.Texts.format(
        ...
        PAYMENT_PROVIDER_TITLE=_settings.payment_zibal.menu_title,
        MINIMUM_PAY_AMOUNT=_settings.payment_zibal.min_pay_amount,
    )
...
"""
    await state.set_state(ZibalCustomAmountForm.amount)
