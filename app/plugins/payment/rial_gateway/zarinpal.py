...
class Settings(BaseSettings):
    ...
    menu_title: str = "درگاه ریالی zarinpal"
...
from .clients import GatewayError, ZarinpalAPI

router = Router(name="payment/zarinpal")
...
class SettingsKeyboard(InlineKeyboardBuilder):
    class Callback(CallbackData, prefix="set_zarinpal"):
        field: Fields
...
class TextsKeyboard(InlineKeyboardBuilder):
    class Callback(CallbackData, prefix="txt_zarinpal"):
...
class ZarinpalEditForm(StatesGroup):
    api_key = State()
    menu_title = State()
...
class ZarinpalCustomAmountForm(StatesGroup):
    method = State()
    amount = State()


@router.message(
    StateFilter(ZarinpalEditForm),
    IsSuperUser(),
...
async def edit_settings(
    ...
    if _settings.enabled:
        ...
        text = "درگاه zarinpal غیرفعال شد!"
    else:
        _settings.enabled = True
        text = "درگاه zarinpal فعال شد!"
    await settings.Settings.update(payment_zarinpal=_settings)
...
async def edit_settings_texts(
    ...
    if callback_data.field == Fields.text_choose_amount:
        ed_text = _texts.payment_zarinpal.choose_amount
    elif callback_data.field == Fields.text_show_invoice:
        ed_text = _texts.payment_zarinpal.show_invoice
    else:
...
async def texts_reset(
    ...
    _texts = texts.get_texts().payment_zarinpal
    if callback_data.field == Fields.text_choose_amount:
    ...
    await texts.Texts.update(payment_zarinpal=_texts)
...
async def texts_edit(
    ...
    if callback_data.field == Fields.text_choose_amount:
        await state.set_state(ZarinpalEditForm.text_choose_amount)
    elif callback_data.field == Fields.text_show_invoice:
        await state.set_state(ZarinpalEditForm.text_show_invoice)
    else:
        return await query.answer(
...
@router.message(
    ZarinpalEditForm.text_choose_amount,
...
@router.message(
    ZarinpalEditForm.text_show_invoice,
    ~F.text.in_([CancelFormAdmin.cancel]),
    ~CommandStart(),
    ~Command("menu"),
    IsSuperUser(),
...
async def edit_settings(  # noqa: F811
    ...
    if callback_data.field == Fields.api_key:
        ...
        await state.set_state(ZarinpalEditForm.free_after)
        ...
        await state.set_state(ZarinpalEditForm.free_after_percent)
    else:
        return await query.answer(
...
@router.message(
    ZarinpalEditForm.min_pay_amount,
    ~F.text.in_([CancelFormAdmin.cancel]),
    ~CommandStart(),
    ~Command("menu"),
    IsSuperUser(),
...
@router.message(
    ZarinpalEditForm.free_after_percent,
    ~F.text.in_([CancelFormAdmin.cancel]),
    ~CommandStart(),
    ~Command("menu"),
    IsSuperUser(),
...
@router.message(
    ...
    ~CommandStart(),
    ~Command("menu"),
    StateFilter(ZarinpalCustomAmountForm),
)
@router.callback_query(
...
async def select(qmsg: CallbackQuery | Message, user: User, state: FSMContext = None):
    ...
    if not _settings.payment_zarinpal.enabled or not _settings.payment_zarinpal.api_key:
        text = f"🚫 در حال حاضر امکان پرداخت به وسیله {_settings.payment_zarinpal.menu_title} غیرفعال می‌باشد!"
        if isinstance(qmsg, CallbackQuery):
    ...
    text = texts.Texts.format(
        ...
        PAYMENT_PROVIDER_TITLE=_settings.payment_zarinpal.menu_title,
        MINIMUM_PAY_AMOUNT=_settings.payment_zarinpal.min_pay_amount,
    )
...
"""
    await state.set_state(ZarinpalCustomAmountForm.amount)
...
@router.message(
    ...
    ~CommandStart(),
    ~Command("menu"),
    ZarinpalCustomAmountForm.amount,
)
...
async def select_amount(
    ...
    try:
        ...
        markup = PayUrl(url=payment.url).as_markup()
        if isinstance(qmsg, CallbackQuery):
