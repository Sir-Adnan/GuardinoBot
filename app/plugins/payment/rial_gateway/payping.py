...
class Settings(BaseSettings):
    ...
    menu_title: str = "درگاه ریالی payping"
...
from .clients import GatewayError, PaypingAPI

router = Router(name="payment/payping")
...
class SettingsKeyboard(InlineKeyboardBuilder):
    class Callback(CallbackData, prefix="set_payping"):
        field: Fields
...
class TextsKeyboard(InlineKeyboardBuilder):
    class Callback(CallbackData, prefix="txt_payping"):
...
class PaypingEditForm(StatesGroup):
    api_key = State()
    menu_title = State()
...
class PaypingCustomAmountForm(StatesGroup):
    method = State()
    amount = State()


@router.message(
    StateFilter(PaypingEditForm),
    IsSuperUser(),
...
async def edit_settings(
    ...
    if _settings.enabled:
        ...
        text = "درگاه payping غیرفعال شد!"
    else:
        _settings.enabled = True
        text = "درگاه payping فعال شد!"
    await settings.Settings.update(payment_payping=_settings)
...
async def edit_settings_texts(
    ...
    if callback_data.field == Fields.text_choose_amount:
        ed_text = _texts.payment_payping.choose_amount
    elif callback_data.field == Fields.text_show_invoice:
        ed_text = _texts.payment_payping.show_invoice
    else:
...
async def texts_reset(
    ...
    _texts = texts.get_texts().payment_payping
    if callback_data.field == Fields.text_choose_amount:
    ...
    await texts.Texts.update(payment_payping=_texts)
...
async def texts_edit(
    ...
    if callback_data.field == Fields.text_choose_amount:
        await state.set_state(PaypingEditForm.text_choose_amount)
    elif callback_data.field == Fields.text_show_invoice:
        await state.set_state(PaypingEditForm.text_show_invoice)
    else:
        return await query.answer(
...
@router.message(
    PaypingEditForm.text_choose_amount,
...
@router.message(
    PaypingEditForm.text_show_invoice,
    ~F.text.in_([CancelFormAdmin.cancel]),
    ~CommandStart(),
    ~Command("menu"),
    IsSuperUser(),
...
async def edit_settings(  # noqa: F811
    ...
    if callback_data.field == Fields.api_key:
        ...
        await state.set_state(PaypingEditForm.free_after)
        ...
        await state.set_state(PaypingEditForm.free_after_percent)
    else:
        return await query.answer(
...
@router.message(
    PaypingEditForm.min_pay_amount,
    ~F.text.in_([CancelFormAdmin.cancel]),
    ~CommandStart(),
    ~Command("menu"),
    IsSuperUser(),
...
@router.message(
    PaypingEditForm.free_after_percent,
    ~F.text.in_([CancelFormAdmin.cancel]),
    ~CommandStart(),
    ~Command("menu"),
    IsSuperUser(),
...
@router.message(
    ...
    ~CommandStart(),
    ~Command("menu"),
    StateFilter(PaypingCustomAmountForm),
)
@router.callback_query(
...
async def select(qmsg: CallbackQuery | Message, user: User, state: FSMContext = None):
    ...
    if not _settings.payment_payping.enabled:
        text = f"🚫 در حال حاضر امکان پرداخت به وسیله {_settings.payment_payping.menu_title} غیرفعال می‌باشد!"
        if isinstance(qmsg, CallbackQuery):
    ...
    text = texts.Texts.format(
        ...
        PAYMENT_PROVIDER_TITLE=_settings.payment_payping.menu_title,
        MINIMUM_PAY_AMOUNT=_settings.payment_payping.min_pay_amount,
    )
...
"""
    await state.set_state(PaypingCustomAmountForm.amount)
...
@router.message(
    ...
    ~CommandStart(),
    ~Command("menu"),
    PaypingCustomAmountForm.amount,
)
...
async def select_amount(
    ...
    try:
        async with in_transaction():
            ...
            _payment = await PaypingAPI.create(
                amount=callback_data.amount,
                transaction_id=transaction.id,
                payer_name=qmsg.from_user.full_name,
                description=f"origin={(await qmsg.bot.me()).id},order_id={transaction.id},user_id={user.id}",
            )
            await RialGatewayPayment.create(
                provider=RialGatewayPayment.Provider.payping,
                data=_payment.model_dump(mode="json"),
                transaction=transaction,
            )
        _texts = texts.get_texts().payment_payping
        text = texts.Texts.format(
        ...
        markup = PayUrl(url=_payment.url).as_markup()
        if isinstance(qmsg, CallbackQuery):
