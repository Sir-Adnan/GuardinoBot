...
class Settings(BaseSettings):
    ...
    menu_title: str = "💸 ارز دیجیتال (swapino)"


class ChooseAmountText(TextValue):
    value: str = """
✔️ شما در حال افزایش اعتبار به وسیله درگاه پرداخت ریالی-ترونی سواپینو هستید!
...
💲مبلغ قابل پرداخت: <b>{AMOUNT_TOMAN}</b> تومان <code>({AMOUNT_DOLLARS} دلار)</code>
...
router = Router(name="payment/swapino")
...
class SettingsKeyboard(InlineKeyboardBuilder):
    class Callback(CallbackData, prefix="set_swapino"):
        field: Fields
...
class TextsKeyboard(InlineKeyboardBuilder):
    class Callback(CallbackData, prefix="txt_swapino"):
...
class SwapinoEditForm(StatesGroup):
    menu_title = State()
...
class SwapinoCustomAmountForm(StatesGroup):
    method = State()
    amount = State()


@router.message(
    StateFilter(SwapinoEditForm),
    IsSuperUser(),
    ~CommandStart(),
    ~Command("menu"),
...
async def show_settings(
    ...
    if (state is not None) and (await state.get_state() is not None):
        await state.clear()
    _settings = settings.get_settings().payment_swapino
    text = f"""
...
async def edit_settings(
    ...
    if _settings.enabled:
        ...
        text = "درگاه swapino غیرفعال شد!"
    else:
        _settings.enabled = True
        text = "درگاه swapino فعال شد!"
    await settings.Settings.update(payment_swapino=_settings)
...
async def edit_settings_texts(
    ...
    if callback_data.field == Fields.text_choose_amount:
        ed_text = _texts.payment_swapino.choose_amount
    elif callback_data.field == Fields.text_show_invoice:
        ed_text = _texts.payment_swapino.show_invoice
    else:
...
async def texts_reset(
    ...
    _texts = texts.get_texts().payment_swapino
    if callback_data.field == Fields.text_choose_amount:
    ...
    await texts.Texts.update(payment_swapino=_texts)
...
async def texts_edit(
    ...
    if callback_data.field == Fields.text_choose_amount:
        await state.set_state(SwapinoEditForm.text_choose_amount)
    elif callback_data.field == Fields.text_show_invoice:
        await state.set_state(SwapinoEditForm.text_show_invoice)
    else:
        return await query.answer(
...
@router.message(
    SwapinoEditForm.text_choose_amount,
...
@router.message(
    SwapinoEditForm.text_show_invoice,
    ~F.text.in_([CancelFormAdmin.cancel]),
    ~CommandStart(),
    ~Command("menu"),
    IsSuperUser(),
...
async def edit_settings(  # noqa: F811
    ...
    if callback_data.field == Fields.min_pay_amount:
        ...
        await state.set_state(SwapinoEditForm.min_pay_amount)

    elif callback_data.field == Fields.menu_title:
        text = "مقدار جدید نام مستعار را وارد کنید:"
        await state.set_state(SwapinoEditForm.menu_title)
        ...
        await state.set_state(SwapinoEditForm.free_after)
        ...
        await state.set_state(SwapinoEditForm.free_after_percent)
    else:
        return await query.answer(
...
@router.message(
    SwapinoEditForm.min_pay_amount,
    ~F.text.in_([CancelFormAdmin.cancel]),
    ~CommandStart(),
    ~Command("menu"),
    IsSuperUser(),
...
@router.message(
    SwapinoEditForm.free_after_percent,
    ~F.text.in_([CancelFormAdmin.cancel]),
    ~CommandStart(),
    ~Command("menu"),
    IsSuperUser(),
...
@router.message(
    ...
    ~CommandStart(),
    ~Command("menu"),
    StateFilter(SwapinoCustomAmountForm),
)
@router.callback_query(
...
async def select(qmsg: CallbackQuery | Message, user: User, state: FSMContext = None):
    ...
    try:
        if not _settings.payment_swapino.enabled or not await NowPaymentsAPI.status():
    ...
    try:
        ...
        text = texts.Texts.format(
            ...
            PAYMENT_PROVIDER_TITLE=_settings.payment_swapino.menu_title,
            USDT_RATE=usdt_rate,
            MINIMUM_PAY_AMOUNT=_settings.payment_swapino.min_pay_amount,
        )
...
"""
    await state.set_state(SwapinoCustomAmountForm.amount)
...
@router.message(
    ...
    ~CommandStart(),
    ~Command("menu"),
    SwapinoCustomAmountForm.amount,
)
...
async def select_amount(
    ...
    try:
        async with in_transaction():
            ...
            np_payment = await NowPaymentsAPI.create_payment(
            ...
            await CryptoPayment.create(
                usdt_rate=usdt_rate,
                payment_id=np_payment.payment_id,
                order_id=np_payment.order_id,
                price_amount=np_payment.price_amount,
                price_currency=np_payment.price_currency,
                nowpm_created_at=np_payment.created_at,
                nowpm_updated_at=np_payment.updated_at,
                provider=CryptoPayment.Provider.swapino,
                transaction=transaction,
            )
        _texts = texts.get_texts().payment_swapino
        text = texts.Texts.format(
            ...
            AMOUNT_DOLLARS=np_payment.price_amount,
        )
        link = f"https://t.me/SwapinoBot?start=BuyTron-{np_payment.pay_address}-{str(np_payment.pay_amount).replace('.', '_')}-Tron"
        markup = PayUrl(url=link).as_markup()
        if isinstance(qmsg, CallbackQuery):
        ...
        if "AMOUNT_MINIMAL_ERROR" in str(err):
            if isinstance(qmsg, CallbackQuery):
                text = "📍 مبلغ پرداختی کمتر از حد مجاز پرداخت در این لحظه می‌باشد!"
