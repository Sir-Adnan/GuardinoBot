...
- **Next:** §9 menu areas all covered (Dashboard·Users·Subscriptions·Services·Discounts·Payments·Panels·Reports·Resellers·Automation·Settings). Remaining = heavier write actions (create/edit services, renew/add-traffic, balance adjust, web-initiated text broadcast — needs a cross-process worker trigger) + more write actions (create/edit services, renew/add-traffic, balance adjust, payment approve/reject), each a FastAPI router + a Refine page through the adapter.
