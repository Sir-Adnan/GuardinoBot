...
- **Backend** `app/api/` (FastAPI) = the `api` compose service (uvicorn :8000), sharing DB/Redis via `config.TORTOISE_ORM` + the §6 adapter. **Telegram-OTP → JWT** auth (`security.py` + `routers/auth.py`, reseller+ only; OTP in Redis, sent via a send-only Bot in `clients.py`); routers (all **read-only** so far): `dashboard`, `users`, `proxies`, `services`, `servers` (+ live `/servers/{id}/health` via `build_panel`), `transactions` — reseller subtree scoping (`deps.require_role`/`_scope`) where applicable; servers/services are admin+, credentials never exposed. New py deps: `fastapi`, **`uvicorn` (NOT `[standard]` — it pulls uvloop, which breaks `aerich` in prestart)**, `pyjwt`.
- **Frontend** `webpanel/` (Vite + React + TS + **Refine + AntD**, RTL, emerald theme, fully responsive) served by nginx (`webpanel` service :8080, proxies `/api` → api so no CORS). Providers in `webpanel/src/providers/` (axios JWT+refresh · authProvider OTP · custom dataProvider for `{items,total}`); responsive shell `components/Layout.tsx` (role-gated menu); pages: Login, Dashboard, + list pages Users (with detail), Subscriptions, Services, Payments, Panels. Needs `npm install` + build (untested).
- **Next (vertical slices):** remaining §9 areas (Resellers, Discounts, Automation/Broadcast, Reports, Settings) + **write actions** (create/edit/renew/refund/disable — routers are read-only so far), each a FastAPI router + a Refine page through the adapter.
